...
class System(resource.Resource):
    ...
    def render_POST(self, request):
        if "ack" in request.args:
            self.level = NONE
            self.oaf.statusChange(self)
        return self.render_GET(request)

    def render_GET(self, request):
        if "status" in request.args and (request.args["status"][0] != ""):
            if ("message" in request.args) and (request.args["message"][0] != ""):
...
class OafServer(resource.Resource):
    ...
    def getChild(self, path, request):
        ...
        self.putSystem(path_str, self.defaultSystemType(path_str, self))
