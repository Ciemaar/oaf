...
def delayCount():
    ...
    if burstPeriod is None:
        ...
        while burstCountDown > 0:
            ...
            if (burstDelay < 0.001) and (zeroDelayCount > burstCount):
    ...
    if len(sys.argv) > 2:
        ...
        if len(sys.argv) < 3:
            ...
            if testPattern in ("1", "4"):
                ...
                transSet = list(range(0, 3))
                transRates = list(range(0, 0xFF))
                if testPattern == "1":
                    pulseSet = list(range(0, 4))
                else:
                    pulseSet = (0,)
                ...
                colorSet = list(range(0, 0xFFFFFF))
                transSet = list(range(0, 3))
                transRates = list(range(0, 0xFF))
                pulseSet = list(range(0, 4))
                normalDelay = 2.0
        ...
        root = resource.Resource()
        orbRoot = OaF.OafServer(OaF.System)
        orbRoot.putSystem("WilhelmPickle", OaF.PickledSystem("http://localhost:8000/oaf/pickle"))
