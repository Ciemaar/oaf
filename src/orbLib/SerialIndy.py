                ...
                for blue_offset in (0x00, 0x40, 0x80, 0xC0):
                    for green_offset in (0x00, 0x40, 0x80, 0xC0):
                        for red_offset in (0x00, 0x40, 0x80, 0xC0):
                            yield (red + red_offset) * 0x010000 + (
                                        green + green_offset) * 0x0100 + (
                                              blue + blue_offset)
...
class SerialIndyNotifier(OaF.Notifier):
    ...
    def updateSLED(self):
        statusCode = OaF.getWebColor(
            self.state["color"]) + "000%01X\r" % round(
            2.0 * self.state["blink"] / 9.0)
        print statusCode
        try:
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) > 2):
                ...
                for trans in transSet:
                    for pulse in pulseSet:
                        for color in colorSet:
                            currSet = "#%06X%01X%02X%01X\r" % (
                            color, trans, transRate, pulse)
                            if (portname != "/dev/null"):
                                ser.write(currSet)
                            print currSet
                            try:
        ...
        orbRoot.putSystem("WilhelmPickle",
                          OaF.PickledSystem("http://localhost:8000/oaf/pickle",
                                            orbRoot))
