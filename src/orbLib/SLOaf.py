...
class SLNotifier(OaF.Notifier):
    ...
    def setState(self, color, blink, message, level, status):
        ...
        if (hasattr(self, "SLChannel")):
            ...
            self.xmlProxy.callRemote("llRemoteData",
                                     {"Channel": self.SLChannel,
                                      "StringValue": self._SLCSV(),
                                      "IntValue": 42}) \
                .addErrback(self.setStateFailed).addCallback(
                self.setStateSuccess)

    def _SLCSV(self):
        return '%s,%d,%s,%f,"%s"' % (
        self.color, self.blink, self.status, self.level, self.message)
...
class SLServer(OaF.SubServer):
    ...
    def __init__(self, systemName, avuuid=None, oaf=None, watchedUrls=None):
        ...
        for type in (
        OaF.System, OaF.CountSystem, OaF.GoalSystem, OaF.GoalNetworkSystem):
            for x in range(0, 4):
    ...
    def updatePageMonitors(self):
        ...
        for url in self.watchedUrls:
            if (url not in curr):
                self.putSystem("PageMonitor%d" % self.monitorCount,
                               PageMonitor(url))

    def updateFromRequest(self, request):
        if request.args.has_key('config') and int(
                request.args['config'][0]) > self.configVersion:
...
class BoundSLOafServer(SLServer):
    def __init__(self, configData, *args, **kwargs):
        super(BoundSLOafServer, self).__init__("loading from db",
                                               configData.AVUUID,
                                               watchedUrls=[x.url for x in
                                                            configData.pagemonitors],
                                               *args, **kwargs)
...
if __name__ == "__main__":
    ...
    oafRoot.putSystem("yahoo", OaF.PageMonitor("http://yahoo.com/",
                                               allowedErrors=("401",)))
    oafRoot.putSystem("litfactory",
                      OaF.PageMonitor("http://localhost:8813/bookstore/Wilson",
                                      "Wilhelm Lit", allowedErrors=("405",)))
    ...
    slSub = OaF.ScaledSubServer("Second Life systems", oafRoot,
                                OaF.CountSystem, 1)
